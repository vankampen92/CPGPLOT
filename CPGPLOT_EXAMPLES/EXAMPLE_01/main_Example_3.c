 ...
 */
 /*
   ...
   ~$ ./PLOT -G0 1 -G1 1 -G2 1 -G3 0 -G4 20.0 -G5 1 -G6 0.0 -G7 100.0 
...
#include "default.c"
 ...
 /* Command line arguments */
  ...
  /* printf("\n");                                    */
  double Delta_Threshold_Min, Delta_Threshold_Max;
  ...
  Delta_Threshold_Min = CPG->CPG_RANGE_X_0 + Offset;  
  Delta_Threshold_Max = CPG->CPG_RANGE_X_1;
  assert(Delta_Threshold_Max > Delta_Threshold_Min); 

  /* B E G I N : Generating 1st Curve Values */
  for(n=0; n<No_of_POINTS_MAX; n++) {
    x_Data[0][n] = Delta_Threshold_Min + (double)n / (double)No_of_POINTS_MAX * (Delta_Threshold_Max - Delta_Threshold_Min);
    y_Data[0][n] = ( 1.0 + x_Data[0][n] / (z_0*Eta) ) * DELTA_A;

    Delta  = x_Data[0][n];
    Beta   = y_Data[0][n];

    x_Data[0][n] = 1.0/x_Data[0][n];
  ...
  CPGPLOT___X_Y___P_L_O_T_T_I_N_G___S_A_M_E___P_L_O_T ( CPG, SAME_PLOT,
							No_of_POINTS_MAX,
							x_Data[0], y_Data[0],
							"Propagule Persistence Time (1/\\gd)",
							"Production Rate (\\gb)",
							CPG->Title,
							CPG->CPG_SCALE_X,
							CPG->CPG_SCALE_Y );
  ...
  for(n=0; n<S; n++) {
    x_Data[1][n] = DELTA_MIN + (double)n/(double)(S-1) * (DELTA_MAX - DELTA_MIN);
    y_Data[1][n] = ( 1.0 + x_Data[1][n] / (z_0 * Eta) ) * DELTA_A; 

    Delta  = x_Data[1][n];
    Beta   = y_Data[1][n];

    x_Data[1][n] = 1.0/x_Data[1][n];
