...
#include "default.c"
 ...
 /* Command line arguments */
  ...
  double C, Beta, Eta, P, Delta, Delta_A, z_0, Offset;
  int S;
  ...
  for(n=0; n<No_of_POINTS_MAX; n++) {
    ...
    y_Data[0][n] = z_0 *(x_Data[0][n] + DELTA)/(x_Data[0][n]*(1.0-2.0*P)) *DELTA_A;
  ...
  for(n=0; n<S; n++) {
    ...
    y_Data[1][n] = z_0 *(x_Data[1][n] + DELTA)/(x_Data[1][n]*(1.0-2.0*P)) *DELTA_A;
  ...
  */
  ...
  /* Horizontal */
  x_1 = 0.0;                       y_1 = z_0*DELTA_A/(1.0-2.0*P);
  x_2 = CPG->CPG_RANGE_X_1;        y_2 = z_0*DELTA_A/(1.0-2.0*P);
  SAME_PLOT = 1;
  CPG->color_Index  = 1;
  CPG->type_of_Line = 3;
  CPG->type_of_Width = 4;  
